...
export const WelcomeCard = () => {
  ...
  const handleActivateCartTransformers = async () => {
    const guid = cartTransformGuid.trim();

    if (!guid) {
      setCartTransformMessage({
        success: false,
        text: "Enter a cart transformer GUID before activating.",
      });
      return;
    }

    // Matching the initialization structure of handleSyncProducts
    ...
    try {
      ...
      if (response.ok) {
        // Handle Shopify specific user errors even on a 200 OK response
        const userErrors =
          data?.cartTransformCreate?.userErrors || data?.userErrors || [];
        ...
        if (userErrors.length > 0) {
          ...
          const newTransformId =
            data?.cartTransformCreate?.cartTransform?.id ||
            data?.cartTransform?.id;

          setCartTransformMessage({
            success: true,
            text: `Cart Transformer activated! ID: ${
              newTransformId || "Success"
            }`,
          });
        }
      } else {
        setCartTransformMessage({
          ...
          text:
            data.error ||
            data.message ||
            "Failed to activate Cart Transformer.",
        });
      }
    } catch (err) {
      setCartTransformMessage({
        success: false,
        text:
          err.message || "An error occurred while activating Cart Transformer.",
      });
    } finally {
      ...
      <Layout>
                ...
                <InlineStack align="space-between">
                  <BlockStack gap="100">
                    ...
                    <Text as="p" tone="subdued">
                      Instantly update your product prices by syncing with SAP
                      data source.
                    </Text>
                  </BlockStack>
                </InlineStack>
                ...
                <Banner tone="success" onDismiss={() => setSyncResult(null)}>
                  <BlockStack gap="400">
                    ...
                    <InlineStack gap="300">
                      ...
                      <Badge tone="info">
                        Total Variants: {syncResult.totalVariants}
                      </Badge>
                      <Badge tone="info-keppel">
                        SKUs: {syncResult.uniqueSKUs}
                      </Badge>
                    </InlineStack>
        ...
        <Layout.AnnotatedSection
          ...
          description={
            <BlockStack gap="400">
              <Text as="p" tone="subdued">
                Manage your secure Redis connection strings. These credentials
                are used to power the real-time pricing engine for your shop.
              </Text>
              {shopName && (
                <Box paddingBlockStart="200">
                  <Badge tone="success-strong">
                    Connection Active: {shopName}
                  </Badge>
                </Box>
              )}
            </BlockStack>
          ...
          <Card>
            <BlockStack gap="500">
              ...
              <BlockStack gap="200">
                ...
                >
                  <span style={{ color: "var(--p-color-text-info)" }}>
                    redis://
                  </span>
                  <span
                    style={{
                      borderBottom: "1px dashed var(--p-color-text-subdued)",
                    }}
                  >
                    {redisUsername || DEMO.username}
                  </span>
                ...
                <InlineStack gap="100" align="center">
                  <Text variant="bodySm" tone="subdued">
                    This URL is generated dynamically for preview purposes.
                  </Text>
                </InlineStack>
              </BlockStack>
              ...
              {/* <BlockStack gap="300">
                ...
                <Button
                  ...
                  disabled={!cartTransformGuid.trim()}
                >
